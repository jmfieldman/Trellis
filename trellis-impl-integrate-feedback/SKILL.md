...
- The fix does not implicitly resolve a question the plan currently lists as open at any scope. Resolving Open questions is a separate planning round.
...
When the review item is real but resolving it requires a human judgment call, new domain knowledge, or design / planning work beyond mechanical reconciliation. File the question at the **right scope** — overview-level Open questions for cross-sprint or feature-wide questions; sprint-level Open questions for sprint-scoped questions. Typical signals:

- The reviewer offers multiple alternatives without one being obviously right (the "Suggested directions" block lists Options A/B/C).
- The fix requires information not in the plan (a sample external payload, an ops confirmation, a stakeholder decision, a peer-service contract).
- The fix would change the shape of a foundational decision, the public surface, the schema, the lifecycle invariants, or the cross-sprint dependency graph.
...
- The fix is "re-slice these sprints" — re-slicing is a planning-round move.
- The fix is a category of risk (rollout, observability, partial-failure recovery, hardening) the plan has not yet sectioned for. Adding the section is design work; *naming* the gap is an open question.
- The fix recommends locking a sprint's Locked Decisions table beyond what's currently locked, where the missing rows aren't trivially derivable from the design plan + the overview's feature-wide decisions.

When filing an Open question from a review item: capture the question, why it's open, the indicative direction(s) the reviewer suggested (if any — preserve the option menu when present), whether it blocks anything currently in scope, and which sprint(s) it touches. Use the standard Open-question shape from the authoring guide. Do **not** copy the review item verbatim — distill it.
...
Default to **Open question** when the item is a *design-shape or sprint-shape* call — a foundational-decision shift, a schema column change, an API verb choice, a lifecycle invariant, a cross-service contract change, an idempotency-key shape change, a sprint dependency rewire, a re-slicing recommendation. The cost of an unwarranted body edit on a shape call (silently hard-coding a choice the human would have made differently, or rewiring sprint dependencies that contradict a future-round decision) is higher than the cost of one extra Open-question bullet.
...
**Calibration check.** After triaging the whole review, count the buckets across all sources (Executive summary, High-level concerns, Cross-sprint coherence, per-sprint findings, per-step findings, Substance, Minor nits). The shape of a healthy *impl-plan* triage is different from a healthy design-plan triage — impl-plan reviews surface a high concentration of cross-sprint coherence drift (Prerequisite ↔ Deliverable name drift, dependency-graph one-liner alignment, `progress.md` step-list mismatch), most of which are mechanical Minor incorporates. The Minor-incorporate bucket runs heavier for impl plans than for design plans. Use the impl-plan bands below; the design-plan version of this skill uses different bands.

A healthy impl-plan triage is roughly:

- **5–15% Incorporate (full)** — material edits that earn a Decisions-log entry (overview-level for cross-cutting calls; sprint-level for sprint-scoped calls). Material incorporations are the minority of any review.
- **40–60% Minor incorporate** — cross-sprint coherence fixes plus the usual mechanical fixes (typos, marketing-language purges, missing forward-links, stale wording, numbering, `progress.md` ↔ per-sprint Progress reconciliation). This bucket runs heaviest for impl plans because the directory-of-files shape produces many seam-drift findings, most of which are mechanical.
- **15–30% Open question** — sprint-shape questions, re-slicing recommendations, hardening-section gaps, and other judgment calls.
...
Combined Incorporate + Minor incorporate is typically 45–75%.

Out-of-band signals:

- If **Open question is holding more than ~50% of items**, re-triage the boundary cases — most reviewer-surfaced wording / consistency / cross-sprint-coherence drift belongs in Minor incorporate, not Open question. The Cross-sprint coherence section of an impl-plan review is the bucket most likely mis-classified — re-check those first. Default-to-Open-question is a safety net for genuine judgment calls, not a default for everything you'd rather not commit to.
- If **Incorporate (full) is holding more than ~25%**, re-check whether you're making fresh design or sprint-shape calls under the cover of "incorporation." Material incorporations should still be the minority — most reviewer-surfaced findings are either mechanical (Minor incorporate) or judgment calls (Open question).
...
- Each entry: the question (one sentence), why it's open / what makes it hard, indicative direction(s) (preserve the option menu if the reviewer offered one — that's load-bearing planning context, not noise), whether it blocks anything currently in scope, and the sprint(s) it touches.
...
6. **Calibration check.** Count the buckets against the impl-plan bands in § "The triage rule" → "Calibration check" (target ~5–15% Incorporate, ~40–60% Minor, ~15–30% Open question, 0–10% Reviewer-wrong, ~5–15% Ignore). If Open question holds >50%, re-triage boundary cases — most reviewer-surfaced wording / consistency / cross-sprint-coherence drift belongs in Minor incorporate, not Open question. If Incorporate (full) holds >25%, re-check for fresh design or sprint-shape calls hiding under "incorporation." If Reviewer-wrong holds >20%, **stop and surface this to the user** — the review may need to be re-run rather than triaged.
...
A bulleted summary of items filed for the next round. Each bullet: the question (one short phrase) + the scope where it was filed (overview-level / Sprint NN) + a one-sentence reason it's a judgment call rather than a mechanical fix. When the reviewer offered a menu of options, note that the menu was preserved in the entry.
