...
**Reviewer-dispatch fallback.** A resolver returns `reviewer_verdict: not_run` when its harness can't dispatch a nested reviewer (per the [nested-dispatch discovery & fallback rule](../specs/nested-dispatch.md)). For each such proposal, **you** dispatch a `question-reviewer` yourself — from your context, which hasn't seen the resolver's reasoning, so the fresh-context audit is preserved. Build the reviewer's spawn prompt the way the resolver's brief does, passing the reviewer's required inputs: file path, feature root, doc type, the question, **the resolver's proposal** (its verdict, recommended option, the options with pros/cons, basis, and confidence — taken from the returned YAML), the sibling questions, **the upstream decided answers** (the same `question N → decided option` values you passed the resolver when it was serialized after a dependency, or "(none)"), the design plan path, and any additional user instructions. Don't background it. Then fold its verdict in using the same reviewer-verb → `reviewer_verdict` crosswalk the resolver uses ([question-resolver.md](../subagents/question-resolver.md) § "Incorporate the reviewer's verdict") — the full mapping, not a subset. The one place your handling **deliberately differs** from the resolver's is `escalate`: the resolver, holding its own investigation context, may accept the reclassification (`reclassified`); you don't, because you lack that context (see the `escalate` row below).

- **`sound`** — keep the proposal; set `reviewer_verdict: sound`.
- **`revise`** — re-dispatch *that one* resolver with the reviewer's correction as added context. The re-dispatched resolver re-forms the proposal and hands back `reviewer_verdict: revised`. (It will again try to spawn its own reviewer; if it again comes back `not_run`, run this fallback once more, then surface if it still can't be reviewed.)
- **`dissent`** — keep the proposal; attach the reviewer's note to `reviewer_note`; set `reviewer_verdict: dissent`.
- **`escalate`** — you lack the resolver's investigation context to overrule a reclassification, so don't silently apply it (this is the deliberate divergence from the crosswalk's `reclassified` branch). **Surface the reviewer's escalate to the user** as a flagged row in the presentation and let them decide whether to accept the reclassification.
...
**Exception — tuning a serialized upstream.** When you serialized a pair (step 2), the downstream resolver was dispatched with the upstream's *decided answer* as a hard input — but at dispatch time that "decided answer" was only the upstream's **unapproved recommendation**. If the user now tunes that upstream to a different option, the downstream's proposal may rest on a stale premise (e.g., tuning Q3's cursor shape silently invalidates Q5, which paginated against the old shape). For each downstream whose `depends_on` names the tuned upstream: if the change plausibly affects the downstream's answer, **re-dispatch that downstream resolver** with the new upstream answer (the one case in this loop where a tune *does* re-spawn). If you can see the change can't affect it, keep the downstream but **flag the dependency to the user** (`Q5 assumed Q3 = B; you changed Q3 to A — Q5's answer is unaffected because …`) rather than silently carrying the stale-premise proposal. Never integrate a downstream against an upstream answer the user has since overridden without one of these two steps.
...
This is the orchestrator's own editing pass — one coherent round, like `impl-iterate` / `impl-integrate-feedback`. Load the relevant authoring guide before editing: [design-plan.md](../specs/design-plan.md) for a `design` file; [implementation-plan.md](../specs/implementation-plan.md) plus [implementation-plan-mechanics.md](../specs/implementation-plan-mechanics.md) for an `impl` file.
...
2. **Move the question out of Open Questions** into the Decisions log. Never append `(resolved)` in place — entries *move*. Tag the new Decisions-log bullet with the round number `(R<n>)`, bold the decision lead, and include the rationale (the resolver's basis, distilled).
   - For an `impl` file: route by the **nature** of the resolved call, not the file it was asked in (matching `impl-iterate`). A cross-cutting call goes in the feature root's **`decisions.md`**; a sprint-scoped call goes in the **owning sprint file's** Decisions log section. Usually an overview-file question resolves to a cross-cutting call and a sprint-file question to a sprint-scoped one — but not always. When an overview question resolves to a sprint-scoped call, log it in the owning sprint file (and edit that sprint file's body if the call changes it); when a sprint question surfaces a cross-cutting call, log it in `decisions.md`.
     - **Fallback when no owning sprint file exists yet.** An `overview.md` question can resolve to a sprint-scoped call for a sprint that hasn't been decomposed into its own file (early plans where the roster exists but stub sprint files don't, or the owning sprint is still a one-line roster entry with no Decisions log). There's no sprint file to write to. Don't invent one mid-run. Instead log the decision in **`decisions.md`** (tagged with the round, noted as *"sprint-scoped to Sprint NN; promote into that sprint file when it's fleshed out"*) and **flag it in the report** so the future `impl-iterate` round that builds Sprint NN moves it into the sprint's local Decisions log. This keeps the decision durable and audit-tagged rather than dropped on the floor.
   - For a `design` file: the call goes in `design.md`'s own Decisions log.
3. **Multi-file discipline (impl only).** If a resolution renames or reshapes something a sibling file references, follow [implementation-plan-mechanics.md § "Multi-file edit discipline"](../specs/implementation-plan-mechanics.md): edit the file that *owns* the wording, then each consumer, then `grep -rn '<old wording>' <root>` — zero hits is the only acceptable result.
...
- A resolver returning a malformed or missing YAML block — don't guess the proposal. First make **one bounded retry**: ask that same resolver to re-emit exactly the fenced YAML block and nothing else (use the harness's continue/reply mechanism if it has one; otherwise re-dispatch that one resolver fresh — resolvers edit nothing, so a fresh dispatch is safe, just note the re-run). If the retry still fails, treat that one question as left-open / needs-re-dispatch, continue the run with the parseable proposals, and flag any sibling whose `depends_on` references the malformed question (its assumed upstream answer is now missing) rather than integrating that sibling against a guess.
...
- **Don't double-number the round.** This run is at most one round and earns at most one `status.md` (or `design.md` Status) entry — and none at all if it integrated nothing. Re-read the Status log immediately before appending so a concurrently-landed round isn't overwritten or duplicated.
- **Don't run two writers against the same `<root>` concurrently.** Integration is a single-writer editing pass over the plan cluster (`status.md`, `decisions.md`, the target file, consumers). Don't run this skill concurrently with `impl-iterate`, `impl-integrate-feedback`, `impl-execute`, **or a second `resolve-open-questions`** on the same `<root>` — two runs racing on `status.md` round numbering and on the same body sections corrupt the plan. The "one file per invocation" rule scopes which file's Open Questions a run processes; it does **not** make two simultaneous runs safe, because integration of either run may touch shared files (`decisions.md`, `status.md`, `overview.md`). There's no lock; it's an operational expectation.
